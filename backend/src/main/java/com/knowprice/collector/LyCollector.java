 ...
 * 同程旅行价格采集器
 * 
 * <p>实现从同程旅行（https://www.ly.com）采集酒店价格数据的功能。
 ...
 */
@Component
public class LyCollector implements PriceCollector {

    private static final Logger logger = LoggerFactory.getLogger(LyCollector.class);
    
    /**
     * 平台标识符
     */
    private static final String PLATFORM_NAME = "ly";
    
    /**
     * 同程酒店首页URL
     */
    private static final String BASE_URL = "https://www.ly.com";
     ...
     * 执行同程平台价格采集
     * 
     * <p>使用Playwright Chromium浏览器访问同程酒店页面，
     * 解析页面数据获取酒店和房型价格信息。</p>
     * 
     * @param region 目标采集城市
    ...
    public List<PriceRecord> collect(String region, String starRatings) throws Exception {
        logger.info("开始采集同程平台数据，地区: {}, 星级: {}", region, starRatings);
        ...
        try {
            ...
            try (com.microsoft.playwright.Browser browser = playwright.chromium().launch(launchOptions);
                ...
                String searchUrl = buildSearchUrl(region);
                logger.info("访问同程搜索页面: {}", searchUrl);
                ...
                for (HotelData hotelData : hotelList) {
                    if (hotelData.getStarRating() != null && starRatings != null && !starRatings.isEmpty()) {
                        if (!isStarMatch(hotelData.getStarRating(), starRatings)) {
                            continue;
                        }
                    }
                ...
                logger.info("同程平台数据采集完成，共 {} 条记录", records.size());
            }
            
            playwright.close();
            
        } catch (Exception e) {
            logger.error("同程平台数据采集失败: {}", e.getMessage(), e);
            throw e;
        }
     ...
     * @param starRatingsParam 星级筛选参数
     * @return 是否匹配
     */
    private boolean isStarMatch(Integer hotelStar, String starRatingsParam) {
        if (starRatingsParam == null || starRatingsParam.isEmpty()) {
            return true;
        }
        
        try {
            List<Integer> targetStars = parseStarRatings(starRatingsParam);
            return targetStars.contains(hotelStar);
        } catch (Exception e) {
            return true;
        }
    }
     ...
     * 解析星级筛选参数
     * 
     * @param starRatings 星级参数字符串
     * @return 星级列表
     */
    private List<Integer> parseStarRatings(String starRatings) {
        List<Integer> stars = new ArrayList<>();
        if (starRatings == null || starRatings.isEmpty()) {
            return stars;
        }
        
        if (starRatings.startsWith("[")) {
            starRatings = starRatings.substring(1, starRatings.length() - 1);
        }
        
        String[] parts = starRatings.split(",");
        for (String part : parts) {
            try {
                stars.add(Integer.parseInt(part.trim()));
    ...
    private String buildSearchUrl(String region) {
        ...
        return BASE_URL + "/hotel/" + cityCode + "/";
    }

    /**
     * 获取城市对应的同程城市编码
     * 
     * <p>同程使用城市拼音全拼作为城市编码。</p>
     * 
     * @param region 城市名称
     * @return 同程城市编码
     */
            ...
            try {
                ...
                for (com.microsoft.playwright.Locator item : items) {
                    try {
                        ...
                        String address = item.locator(".address").textContent();
                        hotel.setAddress(address != null ? address.trim() : "");
                        
                        String brand = item.locator(".brand").textContent();
                        hotel.setBrand(brand != null ? brand.trim() : "");
                        
                        String starRating = item.locator(".star-level").textContent();
                        ...
                        logger.warn("解析同程酒店信息失败: {}", e.getMessage());
                    }
                }
            } catch (Exception e) {
                logger.warn("未找到同程酒店列表或页面结构变化: {}", e.getMessage());
            }
        ...
        public List<RoomTypeData> extractRoomList(String hotelUrl) {
            ...
            try {
                if (hotelUrl != null && !hotelUrl.isEmpty()) {
                    ...
                    page.waitForSelector(".room-list-item", new com.microsoft.playwright.options.PageWaitForSelectorOptions().setTimeout(10000));
                    
                    List<com.microsoft.playwright.Locator> rooms = page.locator(".room-list-item").all();
                    
                    for (com.microsoft.playwright.Locator room : rooms) {
                        try {
                            RoomTypeData roomData = new RoomTypeData();
                            
                            String roomName = room.locator(".room-name").textContent();
                            roomData.setName(roomName != null ? roomName.trim() : "");
                            
                            String priceStr = room.locator(".price-num").textContent();
                            if (priceStr != null) {
                                String price = priceStr.replaceAll("[^0-9.]", "");
                                roomData.setPrice(Double.parseDouble(price));
                            }
                            
                            String availability = room.locator(".room-status").textContent();
                            roomData.setAvailable(availability != null && !availability.contains("售罄") && !availability.contains("满"));
                            
                            roomList.add(roomData);
                        } catch (Exception e) {
                            logger.warn("解析同程房型信息失败: {}", e.getMessage());
                        }
                    }
                }
            } catch (Exception e) {
                logger.warn("获取同程房型列表失败: {}", e.getMessage());
            }
