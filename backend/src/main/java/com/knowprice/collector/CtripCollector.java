 ...
 * 携程旅行网价格采集器
 * 
 * <p>实现从携程旅行网（https://hotels.ctrip.com）采集酒店价格数据的功能。
 ...
 */
@Component
public class CtripCollector implements PriceCollector {

    private static final Logger logger = LoggerFactory.getLogger(CtripCollector.class);
    
    /**
     * 平台标识符
     */
    private static final String PLATFORM_NAME = "ctrip";
    
    /**
     * 携程酒店首页URL
     */
    private static final String BASE_URL = "https://hotels.ctrip.com";

    /**
     * 是否使用无头模式运行浏览器
     * 
     * <p>无头模式不显示浏览器窗口，适合服务器环境运行。
     * 开发调试时可设置为false查看浏览器操作过程。</p>
     */
     ...
     * 执行携程平台价格采集
     * 
     * <p>使用Playwright Chromium浏览器访问携程酒店页面，
     * 解析页面数据获取酒店和房型价格信息。</p>
     * 
     * @param region 目标采集城市
    ...
    public List<PriceRecord> collect(String region, String starRatings) throws Exception {
        logger.info("开始采集携程平台数据，地区: {}, 星级: {}", region, starRatings);
        ...
        try {
            ...
            try (com.microsoft.playwright.Browser browser = playwright.chromium().launch(launchOptions);
                ...
                logger.info("访问携程搜索页面: {}", searchUrl);
                ...
                logger.info("携程平台数据采集完成，共 {} 条记录", records.size());
            }
            
            playwright.close();
            
        } catch (Exception e) {
            logger.error("携程平台数据采集失败: {}", e.getMessage(), e);
            throw e;
        }
    ...
    private String buildSearchUrl(String region, String starRatings) {
        ...
        if (starRatings != null && !starRatings.isEmpty()) {
            url.append("?star=").append(starRatings);
        }
     ...
     * 页面操作辅助类
     * 
     * <p>封装Playwright页面操作，提供酒店列表和房型列表的解析方法。</p>
     */
    static class PageHelper {
        ...
        public List<HotelData> extractHotelList() {
            ...
            try {
                page.waitForSelector(".hotel_item", new com.microsoft.playwright.options.PageWaitForSelectorOptions().setTimeout(10000));
                
                List<com.microsoft.playwright.Locator> items = page.locator(".hotel_item").all();
                
                for (com.microsoft.playwright.Locator item : items) {
                    try {
                        HotelData hotel = new HotelData();
                        
                        String name = item.locator(".hotel_name").textContent();
                        hotel.setName(name != null ? name.trim() : "");
                        
                        String address = item.locator(".hotel_address").textContent();
                        hotel.setAddress(address != null ? address.trim() : "");
                        
                        String brand = item.locator(".brand_name").textContent();
                        hotel.setBrand(brand != null ? brand.trim() : "");
                        
                        String starRating = item.locator(".star_rating").textContent();
                        if (starRating != null && starRating.contains("星")) {
                            hotel.setStarRating(Integer.parseInt(starRating.replaceAll("[^0-9]", "")));
                        }
                        
                        String detailUrl = item.locator("a").getAttribute("href");
                        hotel.setHotelUrl(BASE_URL + detailUrl);
                        
                        hotelList.add(hotel);
                    } catch (Exception e) {
                        logger.warn("解析酒店信息失败: {}", e.getMessage());
                    }
                }
            } catch (Exception e) {
                logger.warn("未找到酒店列表或页面结构变化: {}", e.getMessage());
            }
        ...
        public List<RoomTypeData> extractRoomList(String hotelUrl) {
            ...
            try {
                if (hotelUrl != null && !hotelUrl.isEmpty()) {
                    ...
                    page.waitForSelector(".room_item", new com.microsoft.playwright.options.PageWaitForSelectorOptions().setTimeout(10000));
                    
                    List<com.microsoft.playwright.Locator> rooms = page.locator(".room_item").all();
                    
                    for (com.microsoft.playwright.Locator room : rooms) {
                        try {
                            RoomTypeData roomData = new RoomTypeData();
                            
                            String roomName = room.locator(".room_name").textContent();
                            ...
                            String availability = room.locator(".availability").textContent();
                            roomData.setAvailable(availability != null && !availability.contains("满"));
                            
                            roomList.add(roomData);
                        } catch (Exception e) {
                            logger.warn("解析房型信息失败: {}", e.getMessage());
                        }
                    }
                }
            } catch (Exception e) {
                logger.warn("获取房型列表失败: {}", e.getMessage());
            }
